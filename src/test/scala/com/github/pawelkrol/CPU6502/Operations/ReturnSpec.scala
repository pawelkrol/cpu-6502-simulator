...
class ReturnSpec extends FunOperationsSpec {
  ...
  private def assertSetPC(stack: Int, add: Int): Unit = {
    context("$%04X = $00, $%04X = $C8".format(stack, stack + 1)) { memoryWrite(stack, 0x00); memoryWrite(stack + 1, 0xc8) } {
      it("sets PC to $C800") { expect2 { operation }.toSetPC((0xc800 + add).toShort) }
    }
  }

  private def assertSetSR: Unit = {
    context("$01FA = %00110110") { memoryWrite(0x01fa, 0x35) } {
      it("sets SR to $35") { expect2 { operation }.toSetSR(0x35) }
    }
  }
  ...
  describe("return from interrupt") {
    describe("implied/stack addressing mode") {
      testOpCode(OpCode_RTI) {
        ...
        it("pulls 3 bytes from stack") { expect2 { operation }.toChange { SP }.from(0xf9).to(0xfc) }

        assertSetPC(stack = 0x01fb, add = 0x00)
        assertSetSR
      }
    }
  }
  ...
  describe("return from subroutine") {
    describe("implied/stack addressing mode") {
      testOpCode(OpCode_RTS) {
        it("uses 6 CPU cycles") { expect2 { operation }.toUseCycles(0x06) }
        it("pulls 2 bytes from stack") { expect2 { operation }.toChange { SP }.from(0xf9).to(0xfb) }

        assertSetPC(stack = 0x01fa, add = 0x01)
      }
    }
  }
}
